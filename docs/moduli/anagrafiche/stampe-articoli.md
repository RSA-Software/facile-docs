...
| **Ordinamento** | | Come ordinare la stampa. `PREDEFINITO` cambia da una stampa all'altra: vedi la nota in fondo. | `PREDEFINITO`, `CODICE`, `DESCRIZIONE`; a seconda della stampa anche `CAT.MERC. E POSIZ.POS`, `UBICAZIONE` o `CODICE FORNITORE` |
...
!!! note "Che cosa è il `PREDEFINITO`"
    ...
    Dipende dalla stampa. Nelle due destinate al banco — etichette per i banconi
    e articoli per la gestione bilance — ordina per **bancone e PLU**: cioè
    nell'ordine in cui gli articoli stanno sulle
